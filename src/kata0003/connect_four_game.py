...
class ConnectFourGame:
    ...
    def __find_connections(self, places):
        if len(places) > 1:
            vector_of_travel = (
            ...
            if 0 <= places[-1][0] < 7 and 0 <= places[-1][1] < 6:
                if self.get_place_value(places[-1]) == self.get_place_value(
                    places[-2]
                ):
                    return self.__find_connections(places + [next_place])

                elif self.get_place_value(places[-1]) == None:
                    return {"places": places[:-1], "next": places[-1]}

            return {"places": places[:-1], "next": None}
            ...
            connection_down = self.__find_connections(places + [next_place])
            # Up
            next_place = (places[0][0], places[0][1] - 1)
            connection_up = self.__find_connections(places + [next_place])
            # Joined
            complete_vertical = {
                "places": (
                    connection_down["places"][::-1][:-1]
                    + connection_up["places"]
                ),
                "next": [connection_down["next"], connection_up["next"]],
            }
            ...
            connection_left = self.__find_connections(places + [next_place])
            # Right
            next_place = (places[0][0] + 1, places[0][1])
            connection_right = self.__find_connections(places + [next_place])
            # Joined
            complete_horizontal = {
                "places": (
                    connection_left["places"][::-1][:-1]
                    + connection_right["places"]
                ),
                "next": [connection_left["next"], connection_right["next"]],
            }
            ...
            connection_left_down = self.__find_connections(
                places + [next_place]
            )
            # Right + up
            next_place = (places[0][0] + 1, places[0][1] - 1)
            connection_right_up = self.__find_connections(places + [next_place])
            # Joined
            complete_up_right = {
                "places": (
                    connection_left_down["places"][::-1][:-1]
                    + connection_right_up["places"]
                ),
                "next": [
                    connection_left_down["next"],
            ...
            # Left + up
            next_place = (places[0][0] - 1, places[0][1] - 1)
            connection_left_up = self.__find_connections(places + [next_place])
            # Right + down
            next_place = (places[0][0] + 1, places[0][1] + 1)
            connection_right_down = self.__find_connections(
                places + [next_place]
            )
            # Joined
            complete_down_right = {
                "places": (
                    connection_left_up["places"][::-1][:-1]
                    + connection_right_down["places"]
                ),
                "next": [
                    connection_left_up["next"],
    ...
    def check_winner(self):
        ...
        for place in self.__places_to_check:
            connections = self.__find_connections([place])

            for connection in connections:
                if len(connection["places"]) == 3:
                    next_to_check += [
                        next for next in connection["next"] if next is not None
                    ]
